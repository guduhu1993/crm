...
from model.businesses import BusinessInDB
...
from model.providers import ProvidersInDB
...
def get_providers_info(current_user_id: int, db):
    return db.query(dbProvider).filter(dbProvider.source == current_user_id).all()


def add_provider_info(form_data: ProvidersInDB, current_user_id, db):
    form_data.source = current_user_id
    db_provider = dbProvider(**form_data.dict())
    db.add(db_provider)
    db.commit()
...
def modify_provider_info(form_data: ProvidersInDB, current_user_id: int, db):
    db_provider = db.query(dbProvider).filter(dbProvider.provider_name == form_data.provider_name).first()
    print(current_user_id)
    print(db_provider.source)
    if current_user_id != db_provider.source:
        print("无权修改其他来源的信息")
...
def delete_provider_info(provider_id: int, current_user_id: int, db):
    if provider_id != NULL:
...
def get_businesses_info(current_user_id: int, db):
    return db.query(dbBusiness).filter(dbBusiness.source == current_user_id).all()


def add_business_info(form_data: BusinessInDB, current_user_id, db):
    form_data.source = current_user_id
...
def modify_business_info(form_data: BusinessInDB, current_user_id: int, db):
    ...
    print(current_user_id)
    print(db_business.source)
    if current_user_id != db_business.source:
        print("无权修改其他来源的信息")
        raise HTTPException(status_code=401, detail="Item not found")
...
def delete_business_info(business_id: int, current_user_id: int, db):
    print(business_id)
    if business_id != NULL:
...
#######################################################################################################
# 证书
def add_certificate_info(form_data: CertificateInDB, db):
...
def get_certificates_info(certificate_belong_to: int, certificate_belong_type: bool, db):
...
def delete_certificate_info(certificate_id: int, db):
...
#######################################################################################################
# 合同
def add_contract_info(form_data: ContractInDB, current_user_id, db):
    form_data.source = current_user_id
...
def get_contract_info(current_user_id: int, db):
    return db.query(dbContract).filter(dbContract.source == current_user_id).all()
...
def modify_contract_info(form_data: ContractInDB, current_user_id: int, db):
    ...
    if current_user_id != db_contract.source:
        print("无权修改其他来源的信息")
        raise HTTPException(status_code=401, detail="Item not found")
...
def delete_contract_info(contract_id: int, current_user_id: int, db):
    if contract_id != NULL:
...
def add_payin_info(form_data: PayInInDB, db):
...
def delete_payin_info(payin_id: int, db):
    if payin_id != NULL:
...
def add_payout_info(form_data: PayOutInDB, db):
...
def delete_payout_info(payout_id: int, db):
    if payout_id != NULL:
